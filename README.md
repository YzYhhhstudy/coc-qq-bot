...
| 玩法 | `玩法-流派推荐-17` | 该大本流行流派列表（含一键复制配兵链接），meta 数据人工月更 |
| | `玩法-防御阵型推荐-17` | 阵型站分级页链接（Blueprint 月更 CWL 阵、黑羽COC、B站视频等）+ 公众号精选文章（人工收录，把好文链接发给 Claude 即可加入） |
| | `玩法-个性化阵型推荐-17` | 艺术/整活阵合集（皮卡丘/爱心/文字阵） |
...
- `app/ws_main.py` — **主入口**：WebSocket 连接、私聊/群聊事件、每日成长快照任务

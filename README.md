...
**Q：怎么知道机器人有没有更新？** 发「版本」查询。默认开启**自动更新**：每小时比对 GitHub，有新版本自动拉代码+重启（约 1 分钟内完成，期间消息可能漏回）。注意这意味着 main 分支上的代码会自动跑到你的部署机上——合并 PR 前务必审查。不放心就设 `AUTO_UPDATE=0` 改为手动 update.bat。
...
**日常更新——通常不用管**：机器人每小时自动检查 GitHub，有新版本会**自动更新并重启**（`.env` 里 `AUTO_UPDATE=0` 可关闭）。

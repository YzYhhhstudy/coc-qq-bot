...
HELP = (
    "🛡️ 部落冲突助手（子功能用 - 连接）\n"
    "【部落】绑定 #TAG｜解绑｜部落 [#TAG]｜成员 [#TAG]｜战绩 [#TAG]｜战况｜突袭｜捐兵｜搜索 名字\n"
    "【联赛】联赛｜联赛-积分榜｜联赛-奖章｜联赛-对阵｜联赛-2(第2场)｜联赛-催刀｜联赛-敌刀｜联赛-复盘 [场次]｜联赛-总结\n"
    "【玩家】玩家 #TAG｜玩家-英雄 #TAG｜玩家-部队 #TAG｜玩家-法术 #TAG\n"
    "【我】绑定玩家 #TAG｜解绑玩家｜我｜我-英雄｜我-部队｜我-法术｜我-成长\n"
...
async def handle(group_openid: str, content: str) -> str:
    ...
    try:
        ...
        if main in clan_cmds:
            ...
            if main == "战况":
                return _fmt_war(await coc.get_current_war(tag))
            if main == "战绩":
...
def _fmt_members(c: dict) -> str:
    ...
    for i, m in enumerate(c["memberList"][:50], 1):
        ...
        lines.append(f"{i}. {role}{m['name']} 🏆{m['trophies']} 捐{m['donations']}")
    return "\n".join(lines)
    ...
    for w in await _cwl_wars(group):
        ...
        for side in ("clan", "opponent"):
            ...
            for m in w[side].get("members", []):
                p = players.setdefault(m["tag"], {"name": m["name"], "stars": 0, "wars": 0})
                p["wars"] += 1

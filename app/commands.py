...
def _atk_str(m: dict) -> str:
    """单个成员的进攻结果串：⭐⭐⭐100% / 0⭐12% / ✖未打。多刀用｜分隔。"""
    atks = m.get("attacks", [])
    if not atks:
        return "✖未打"
    parts = []
    for a in atks:
        stars = "⭐" * a.get("stars", 0) or "0⭐"
        parts.append(f"{stars}{a.get('destructionPercentage', 0):.0f}%")
    return "｜".join(parts)


def _fmt_progress_pairs(war: dict, title: str) -> str:
    """逐人对位进度：1. AAA ⭐⭐⭐100% VS BBB ⭐49%"""
    us, them = war["clan"], war["opponent"]
    ours = sorted(us.get("members", []), key=lambda m: m.get("mapPosition", 99))
    theirs = sorted(them.get("members", []), key=lambda m: m.get("mapPosition", 99))
    left = _time_left(war)
    lines = [f"{title} | {WAR_STATE.get(war.get('state'), '')}" +
             (f" | {left}" if left else ""),
             f"⭐{us.get('stars', 0)}-{them.get('stars', 0)} | "
             f"{us.get('destructionPercentage', 0):.1f}% - "
             f"{them.get('destructionPercentage', 0):.1f}%"]
    for i, (a, b) in enumerate(zip(ours, theirs), 1):
        lines.append(f"{i}. {a['name']} {_atk_str(a)} VS {b['name']} {_atk_str(b)}")
    return "\n".join(lines)


async def _fmt_cwl_progress(clan_tag: str, group: dict, round_no: int | None) -> str:
    """联赛某场的逐人进度。不指定场次时优先取正在打的那场。"""
    war = None
    if round_no is None:
        for r in reversed(_ready_rounds(group)):
            rn, w, _e = await _find_cwl_war(clan_tag, group, r)
    ...
    if war.get("state") == "preparation":
        return f"第{round_no}场还在备战日（{_time_left(war)}），对位表看：联赛-{round_no}"
    return _fmt_progress_pairs(war, f"📊 联赛第{round_no}场 进度")


def _fmt_war_progress(w: dict) -> str:
    if w.get("state") == "preparation":
        return f"还在备战日（{_time_left(w)}），对位表看：部落战-对阵"
    return _fmt_progress_pairs(w, "📊 部落战进度")
